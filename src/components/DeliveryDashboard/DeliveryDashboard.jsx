...
function DeliveryDashboard() {
  const columns = [
    ...
    },
    {
      ...
      key: 'action',
      render: (text, record) => {
        const isDeliveryDateTBD = record.deliveryDate === "TBD";
        const isPickupDateTBD = record.pickupDate === "TBD";
        
        return (
          <div style={{ display: "flex", alignItems: "center"}}>
            <Button onClick={() => handleDeliveryDateUpdate(record)} style={{ marginRight: "10%" }} disabled={!isDeliveryDateTBD}>
              Register
            </Button>
            <Button onClick={() => handlePickupDateUpdate(record)} disabled={!isPickupDateTBD}>
              Hand Out
            </Button>
          </div>
        );
